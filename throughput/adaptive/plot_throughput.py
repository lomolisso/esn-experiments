...
for i, (gateway_num, gateway_source, cloud_num, cloud_source) in enumerate(file_combinations):
    ...
    gateway_nodes = gateway_num if gateway_num is not None else 0
    axes[i].hist(
        unified_time, bins=len(unified_time), 
        weights=[c + g for c, g in zip(padded_throughput_cloud, padded_throughput_gateway)],
        label=f"{gateway_nodes} Nodes", 
    ...
    axes[i].hist(
        unified_time, bins=len(unified_time), 
        weights=[c + g for c, g in zip(padded_throughput_cloud, padded_throughput_gateway)],
        color="black", stacked=True, histtype='step', linewidth=1.5, zorder=4
    )
    ...
    # Calculate center of the plot for placing text
    x_center = (unified_time[0] + unified_time[-1]) / 2
    y_center = max(axes[i].get_ylim()) / 2
    
    # Annotate the total area in the center of the plot
    axes[i].text(x_center, y_center // 4, f'~{total_area:.0f}Kb', 
    ...
    # set x-label for the second row only
    if i >= 3:
        axes[i].set_xlabel('Time (seconds)')

    axes[i].legend(edgecolor='black')
